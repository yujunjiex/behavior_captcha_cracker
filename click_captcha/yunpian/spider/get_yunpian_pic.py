...
def get_captcha():
    ...
    params = {
        ...
        'i': encrypt_data['i'],
        'k': encrypt_data['k'],
        'captchaId': 'e1e7be036f9242c7aed023438af66f46'  # 固定值
    }
    resp = requests.get(url, params=params, headers=header)
    result = json.loads(resp.text.replace('ypjsonp(', '').replace(')', ''))
    if result['msg'] == 'ok':
        return {
            'captcha_image': result['data']['captchaImage'],
            'words_image': result['data']['wordsImage']
        }
    return None
...
def get_yunpian_pic(count=500):
    ...
    for i in range(count):
        res = get_captcha()
        if res:
            pic_download(res['captcha_image'], '{}captcha'.format(cur))
            pic_download(res['words_image'], '{}words'.format(cur))
            print('{}成功！'.format(cur))
            cur += 1
        else:
            print('{}失败.'.format(cur))
